...
try:
    ...
    for i in os.listdir("./class_left"):
        if count == 5:
            imagesLeft.append(cv2.resize(cv2.imread(f"./class_left/{i}"), dsize=(270, 480)))
            count = 0
        count += 1
    ...
    for i in os.listdir("./class_right"):
        if count == 5:
            imagesRight.append(cv2.resize(cv2.imread(f"./class_right/{i}"), dsize=(270, 480)))
            count = 0
        count += 1
    ...
    for i in os.listdir("./class_false"):
        if count == 5:
            imagesFalse.append(cv2.resize(cv2.imread(f"./class_false/{i}"), dsize=(270, 480)))
            count = 0
        count += 1
...
model.save(f"./trainedmodel.h5", save_format="h5")